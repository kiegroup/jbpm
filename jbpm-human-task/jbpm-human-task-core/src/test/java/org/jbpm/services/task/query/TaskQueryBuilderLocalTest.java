...
//            taskService.getTasksByStatusByProcessInstanceId(procInstId, statuses, "en-UK");
    ...
    private void devTest() {
        ...
        CriteriaQuery<TaskImpl> query = builder.createQuery(TaskImpl.class);
        ...
        query.select(select);
        Join peopleAssignJoin = taskRoot.join(TaskImpl_.peopleAssignments);
        Join potOwnerJoin = peopleAssignJoin.join(PeopleAssignmentsImpl_.potentialOwners, JoinType.LEFT);
        Join busAdminJoin = peopleAssignJoin.join(PeopleAssignmentsImpl_.businessAdministrators, JoinType.LEFT);
        Join stakeHolderJoin = peopleAssignJoin.join(PeopleAssignmentsImpl_.taskStakeholders, JoinType.LEFT);
       
        List<String> groupIds = new ArrayList<String>();
        groupIds.add("Stake Holder");
//        groupIds.add("reviewer");
        List<Predicate> predicates = new ArrayList<Predicate>();
        predicates.add( builder.equal(taskRoot.get(TaskImpl_.taskData).get(TaskDataImpl_.actualOwner).get(UserImpl_.id), stakeHolder) );
        predicates.add( builder.equal(taskRoot.get(TaskImpl_.taskData).get(TaskDataImpl_.createdBy).get(UserImpl_.id), stakeHolder) );
        Path<String> userGroupIdPath = busAdminJoin.get(UserImpl_.id);
        predicates.add( builder.or( userGroupIdPath.in(groupIds), builder.equal(userGroupIdPath, stakeHolder)));
        userGroupIdPath = potOwnerJoin.get(UserImpl_.id);
        predicates.add( builder.or( userGroupIdPath.in(groupIds), builder.equal(userGroupIdPath, stakeHolder)));
        userGroupIdPath = stakeHolderJoin.get(UserImpl_.id);
        predicates.add( builder.or( userGroupIdPath.in(groupIds), builder.equal(userGroupIdPath, stakeHolder)));
      
        if( ! predicates.isEmpty() ) { 
            query.where(builder.or(predicates.toArray(new Predicate[predicates.size()])));
        }
        Query realQuery = em.createQuery(query);
        
        List<TaskSummary> emfRes = realQuery.getResultList();
        assertEquals( "EMF test", 3, emfRes.size() );
    }
