...
public class ExecuteReminderCommand extends TaskCommand<Void> {
	...
	private Notification buildDefaultNotification( TaskData taskData,Task task ){
		EmailNotification  emailNotificationImpl = new EmailNotificationImpl();
		Map<LanguageImpl, EmailNotificationHeaderImpl> map = new HashMap<LanguageImpl, EmailNotificationHeaderImpl>();
		...
		map.put(new LanguageImpl("en-UK"), emailNotificationHeaderImpl);
