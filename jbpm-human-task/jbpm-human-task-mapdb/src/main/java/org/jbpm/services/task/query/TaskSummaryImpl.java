...
public class TaskSummaryImpl implements InternalTaskSummary {
    ...
    public TaskSummaryImpl(Task task) {
    	this(task.getId(), task.getName(), task.getSubject(), task.getDescription(), 
    			task.getTaskData().getStatus(), task.getPriority(), task.getTaskData().isSkipable(), 
    			task.getTaskData().getActualOwner(), task.getTaskData().getCreatedBy(), 
    			task.getTaskData().getCreatedOn(), task.getTaskData().getActivationTime(), 
    			task.getTaskData().getExpirationTime(), task.getTaskData().getProcessId(), 
    			task.getTaskData().getProcessSessionId(), task.getTaskData().getProcessInstanceId(), 
    			task.getTaskData().getDeploymentId(), ((InternalTask) task).getSubTaskStrategy(), 
