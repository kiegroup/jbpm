...
public class AuditQueryCriteriaUtil extends QueryCriteriaUtil {
    ...
    public AuditQueryCriteriaUtil(JPAAuditLogService service) { 
        super(criteriaAttributes);
        this.auditLogService = service;
    }
  
    private EntityManager getEntityManager() { 
        return this.auditLogService.getEntityManager();
    }
  
    private Object joinTransaction(EntityManager em) { 
        return this.auditLogService.joinTransaction(em);
    }
   
    private void closeEntityManager(EntityManager em, Object transaction) {
        this.auditLogService.closeEntityManager(em, transaction);
    }
    ...
    protected <R,T> Predicate implSpecificCreatePredicateFromSingleCriteria( 
        ...
        if( LAST_VARIABLE_LIST.equals(criteria.getListId()) ) {
            Subquery<VariableInstanceLog> maxIdSubQuery = criteriaQuery.subquery(VariableInstanceLog.class);
            ...
            if( varIdField instanceof SingularAttribute ) { 
                expr = table.get((SingularAttribute)varIdField);
            } else { 
