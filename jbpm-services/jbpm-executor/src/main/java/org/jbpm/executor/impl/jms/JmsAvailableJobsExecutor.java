    ...
    public void onMessage(Message message) {
        if (message instanceof TextMessage) {
            ...
            try {
                ...
                RequestInfo request = (RequestInfo) queryService.getRequestById(requestId);
                logger.debug("Found following job request for id {} : {}", requestId, request);
                if (request != null && (STATUS.QUEUED.equals(request.getStatus()) || STATUS.RETRYING.equals(request.getStatus()))) {
