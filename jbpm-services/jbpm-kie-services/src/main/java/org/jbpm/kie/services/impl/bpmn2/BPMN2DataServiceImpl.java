...
public class BPMN2DataServiceImpl implements DefinitionService, DeploymentEventListener {
    ...
    private static final String [] dialectNames = { "java", "mvel" };
    ...
    static { 
        ...
        for( int i = 0; i < dialectNames.length; ++i ) { 
            ProcessDialectRegistry.setDialect(dialectNames[i], dataServiceDialects[i]);
        }
    }
	...
	public ProcessDefinition buildProcessDefinition(String deploymentId,String bpmn2Content, KieContainer kieContainer, boolean cache)
        ...
        try {
	        ...
	        useDataServiceExpressionBuilders(threadLocalHelper);
	...
	private void useDataServiceExpressionBuilders(ProcessDescRepoHelper processHelper) { 
	   for( int i = 0; i < dialectNames.length; ++i ) { 
	       ProcessDialect dialect = ProcessDialectRegistry.getDialect(dialectNames[i]);
	       ...
	       try { 
	           ...
	           if( assignmentBuilder instanceof DataServiceExpressionBuilder ) { 
	               ((DataServiceExpressionBuilder) assignmentBuilder).setThreadLocalHelper(processHelper);
	           }
	       } catch( UnsupportedOperationException uoe ) { 
	           // do nothing
	       }
	       ...
	       try { 
	           ...
	           if( returnValueEvaluatorBuilder instanceof DataServiceExpressionBuilder ) { 
	               ((DataServiceExpressionBuilder) returnValueEvaluatorBuilder).setThreadLocalHelper(processHelper);
	           }
	       } catch( UnsupportedOperationException uoe ) { 
	           // do nothing
	       }
	       ...
	       try { 
	           ...
	           if( processClassBuilder instanceof DataServiceExpressionBuilder ) { 
	               ((DataServiceExpressionBuilder) processClassBuilder).setThreadLocalHelper(processHelper);
	           }
	       } catch( UnsupportedOperationException uoe ) { 
	           // do nothing
	       }
	...
	private void resetDialectExpressionBuilders() { 
	       for( int i = 0; i < dialectNames.length; ++i ) { 
	           ProcessDialect dialect = ProcessDialectRegistry.getDialect(dialectNames[i]);
