...
UPDATE TaskEvent SET workItemId = (SELECT workItemId FROM Task WHERE id = TaskEvent.taskId);
UPDATE TaskEvent SET processInstanceId = (SELECT processInstanceId FROM Task WHERE id = TaskEvent.taskId);
