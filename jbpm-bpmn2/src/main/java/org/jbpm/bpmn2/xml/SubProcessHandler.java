...
public class SubProcessHandler extends AbstractNodeHandler {
    ...
    protected void handleNode(final Node node, final Element element, final String uri, 
        ...
        while (xmlNode != null) {
        	...
        	if ("ioSpecification".equals(nodeName)) {
        		...
        		readDataInputAssociation(xmlNode, (CompositeNode) node, dataInputs);
        	}
    ...
    protected void readDataInputAssociation(org.w3c.dom.Node xmlNode, CompositeNode node, Map<String, String> dataInputs) {
		// sourceRef
		org.w3c.dom.Node subNode = xmlNode.getFirstChild();
		if ("sourceRef".equals(subNode.getNodeName())) {
    		...
    		node.addInAssociation(new DataAssociation(
    				source,
    ...
    public Object end(final String uri, final String localName,
		...
		while (xmlNode != null) {
			...
			if ("multiInstanceLoopCharacteristics".equals(nodeName)) {
				...
				forEachNode.setOutMapping(((CompositeNode) node).getOutMapping());
				node = forEachNode;
				if(forEachNode.getInMapping() == null || forEachNode.getInMapping().size() ==0) {
					handleForEachNode(node, element, uri, localName, parser);
				} else {
					handleForEachNodeWithDataAssociations(node, element, uri, localName, parser);
				}
				found = true;
				break;
			}
	...
	protected void handleForEachNodeWithDataAssociations(final Node node, final Element element, final String uri, 
            final String localName, final ExtensibleXmlParser parser) throws SAXException {
    	ForEachNode forEachNode = (ForEachNode) node;
    	org.w3c.dom.Node xmlNode = element.getFirstChild();
        while (xmlNode != null) {
            String nodeName = xmlNode.getNodeName();
            if ("multiInstanceLoopCharacteristics".equals(nodeName)) {
            	readMultiInstanceLoopCharacteristicsWithDataAssociations(xmlNode, forEachNode, parser);
            }
            xmlNode = xmlNode.getNextSibling();
        }
        List<SequenceFlow> connections = (List<SequenceFlow>)
	...
	protected void readMultiInstanceLoopCharacteristicsWithDataAssociations(org.w3c.dom.Node xmlNode, ForEachNode forEachNode, ExtensibleXmlParser parser) {
        // sourceRef
        org.w3c.dom.Node subNode = xmlNode.getFirstChild();
        while (subNode != null) {
            String nodeName = subNode.getNodeName();
