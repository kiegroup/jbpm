...
public class RuleFlowProcessFactoryBuilder implements ProcessBuilderFactory {
    ...
    @Override
    public RuleFlowProcessFactory processBuilder(String processId) {
        return RuleFlowProcessFactory.createProcess(processId);
    }

    @Override
    public byte[] toBytes(Process process) {
        return dumper.dumpProcess(process).getBytes();
    }
}
