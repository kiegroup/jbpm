...
import org.drools.exception.HandlerRuntimeException;
...
import org.jbpm.workflow.exception.*;
...
public abstract class WorkflowProcessInstanceImpl extends ProcessInstanceImpl implements WorkflowProcessInstance,
    ...
    public void removeNodeInstance(final NodeInstance nodeInstance) {
        if (((NodeInstanceImpl) nodeInstance).isInversionOfControl()) {
            getKnowledgeRuntime().retract(getKnowledgeRuntime().getFactHandle(nodeInstance));
        }
    ...
    public Object getVariable(String name) {
        // for disconnected process instances, try going through the variable
        // scope instances
        // (as the default variable scope cannot be retrieved as the link to the
        // process could
    ...
    public void start() {
        synchronized (this) {
            try {
                ...
                rethrowExceptionToBeHandled(exception);
            } catch (Error error) {
                rethrowExceptionToBeHandled(error);
            }
        }
    }
        ...
        for (Node node : getWorkflowProcess().getNodes()) {
            if (node instanceof EventNode) {
                if ("external".equals(((EventNode) node).getScope())) {
                    addEventListener(((EventNode) node).getType(), new ExternalEventListener(), true);
                }
    ...
    /**
     * This method does three things, in the following order:<ol>
     * <li>It adds the exception thrown to the context. The exception is stored in a list which is stored as the 
     * {@link ExceptionConstants#THROWN_EXCEPTIONS} variable in the context.</li>
     * <li>It makes sure that the {@link NodeInstance#cancel()} has been called on the {@link NodeInstance} that called
     * the exception</li>
     * <li>It attaches a new instance of a {@link JavaExceptionHandler} to the rethrown {@link HandlerRuntimeException}. The
     * {@link JavaExceptionHandler} instance is responsible for handling the exception when if it's caught.</li>
     * </ol>
     * 
     * @param cause The exception thrown. 
     */
    private void rethrowExceptionToBeHandled(Throwable cause) {
        // Add exception to the context
        VariableScopeInstance context = (VariableScopeInstance) getContextInstance(VariableScope.VARIABLE_SCOPE);
        List<Throwable> exceptions = (List<Throwable>) context.getVariable(ExceptionConstants.THROWN_EXCEPTIONS);
        if( exceptions == null ) { 
            exceptions = new ArrayList<Throwable>();
            context.setVariable(ExceptionConstants.THROWN_EXCEPTIONS, exceptions);
        }
        exceptions.add(cause);

        // If an ExceptionProcessEventListener is present, add a JavaExceptionHandler instance
        ExceptionProcessEventListener exceptionListener = getExceptionListener();
        if( exceptionListener != null ) { 
            // get name of error in order to signal exception-handling (error-)event subprocess
            String configuredErrorName 
                = (String) getKnowledgeRuntime().getEnvironment().get(ExceptionConstants.EXCEPTION_ERROR_CODE);
            if (configuredErrorName == null) {
                configuredErrorName = "";
            }

            JavaExceptionHandler exceptionHandler 
                = new ProcessJavaExceptionHandlerImpl(this, configuredErrorName, exceptionListener);
            throw new HandlerRuntimeException(cause, exceptionHandler);
        }
        else { 
    ...
    private ExceptionProcessEventListener getExceptionListener() {
        List<ProcessEventListener> eventListeners = ((InternalProcessRuntime) getKnowledgeRuntime().getProcessRuntime())
                .getProcessEventSupport().getEventListeners();
        ExceptionProcessEventListener exceptionListener = null;
        for (ProcessEventListener eventListener : eventListeners) {
            if (eventListener instanceof ExceptionProcessEventListener) {
                exceptionListener = (ExceptionProcessEventListener) eventListener;
                break;
            }
        }
    ...
    public void addEventListener(String type, EventListener listener, boolean external) {
        ...
        if (listeners == null) {
            ...
            if (external) {
                ((InternalProcessRuntime) getKnowledgeRuntime().getProcessRuntime()).getSignalManager()
                        .addEventListener(type, this);
            }
        }
    ...
    public void removeEventListener(String type, EventListener listener, boolean external) {
        ...
        if (listeners != null) {
            ...
            if (listeners.isEmpty()) {
                ...
                if (external) {
                    ((InternalProcessRuntime) getKnowledgeRuntime().getProcessRuntime()).getSignalManager().removeEventListener(
                            type, this);
                }
            }
        }
    ...
    public void nodeInstanceCompleted(NodeInstance nodeInstance, String outType) {
        if (nodeInstance instanceof EndNodeInstance || ((org.jbpm.workflow.core.WorkflowProcess) getWorkflowProcess()).isDynamic()
