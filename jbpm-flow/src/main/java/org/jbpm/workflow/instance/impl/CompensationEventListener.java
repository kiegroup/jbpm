...
class CompensationEventListener implements EventListener, ProcessImplementationPart {

    private final WorkflowProcessInstanceImpl instance;
    ...
    public boolean isStackless() {
        return this.instance.isStackless();
    }
    ...
    private Stack<NodeInstance> createNodeInstanceContainers(Node toCompensateNode, boolean generalCompensation) {
       ...
       if( nestedNodes.isEmpty() ) {
           ...
           parentInstance = (NodeInstanceContainer) ((WorkflowProcessInstanceImpl) getProcessInstance()).createNodeInstance((Node) nestedNodes.pop());
       ...
       while( ! nestedNodes.isEmpty() ) {
           // generate
           childInstance = (NodeInstanceContainer) parentInstance.createNodeInstance((Node) nestedNodes.pop());
       ...
       if( generalCompensation ) {
           childInstance = (NodeInstanceContainer) parentInstance.createNodeInstance(toCompensateNode);
           generatedInstances.add((NodeInstance) childInstance);
       }
