...
import org.jbpm.workflow.instance.impl.queue.AfterCompensationAction;
...
public class CompensationScopeInstance extends ExceptionScopeInstance  {
    ...
    public void handleException(String activityRef, Object dunno) {
        ...
        if( queueBased ) {
            ...
            // pre-add to queue so it's executed after all other compensation actions..
            processInstance.addProcessInstanceAction(new AfterCompensationAction(this));
            // no true needed because we added something to the queue
        ...
        if (handler instanceof CompensationHandler) {
            ...
            try {
                ...
                if (handlerNode instanceof BoundaryEventNode ) {
                    ...
                    signalCompensationHandler(eventNodeInstance, compensationActivityRef);
    ...
    private void signalCompensationHandler(EventSignallable eventNodeInstance, String compensationActivityRef) {
        String signalType = "Compensation";
        if( isQueueBased() ) {
            getProcessInstance().addSignalEventAction(eventNodeInstance, signalType, compensationActivityRef);
        } else {
            eventNodeInstance.signalEvent(signalType, compensationActivityRef);
        }
    }
