...
public abstract class QueryCriteriaUtil {
    ...
    protected static void addCriteria( Map<Class, Map<String, Attribute>> criteriaAttributes, String listId, Class table, Attribute attr ) {
     ...
     */
    @SuppressWarnings("unchecked")
    protected <R,T> void fillCriteriaQuery( CriteriaQuery<R> criteriaQuery, QueryWhere queryWhere, CriteriaBuilder criteriaBuilder, Class<T> queryType ) {

        Predicate queryPredicate = createPredicateFromCriteriaList(criteriaQuery, queryWhere.getCriteria(), criteriaBuilder, queryType);
        ...
        ServiceLoader<QueryModificationService> queryModServiceLdr = ServiceLoader.load(QueryModificationService.class);
        for( QueryModificationService queryModService : queryModServiceLdr ) {
            queryModService.addCriteriaToQuery(queryWhere, criteriaQuery, criteriaBuilder, queryType);
        }
        ...
        if( queryWhere.getAscOrDesc() != null ) { 
            ...
            Expression orderByPath = getOrderByExpression(orderByListId, queryType, criteriaQuery);
            Order order;
    ...
    private <R,T> Predicate createPredicateFromCriteriaList(CriteriaQuery<R> query, List<QueryCriteria> inputCriteriaList, CriteriaBuilder builder, Class<T> resultType ) {
        ...
        if( inputCriteriaList.size() > 1 ) { 
            ...
            for( QueryCriteria criteria : inputCriteriaList ) {
                ...
                if( criteria.isFirst() ) { 
                   ...
                   continue;
                    ...
                    if( criteria.isUnion() ) { 
                       Predicate predicate = createPredicateFromSingleOrGroupCriteria(query, previousCriteria, builder, resultType);
                ...
                if( criteria.isUnion() ) { 
                    ...
                    Predicate predicate = createPredicateFromSingleOrGroupCriteria(query, criteria, builder, resultType);
            ...
            queryPredicate = createPredicateFromSingleOrGroupCriteria(query, singleCriteria, builder, resultType);
        }
     ...
     */
    private <R,T> Predicate createPredicateFromSingleOrGroupCriteria(CriteriaQuery<R> query, QueryCriteria criteria, CriteriaBuilder builder, Class<T> queryType ) {
        Predicate predicate;
        if( criteria.isGroupCriteria() ) { 
            ...
            predicate = createPredicateFromCriteriaList(query, criteria.getCriteria(), builder, queryType);
        } else { 
            assert ! criteria.hasCriteria() : "Criteria has both values and subcriteria (group criteria)! [" + criteria.toString() + "]";
            predicate = createPredicateFromSingleCriteria(query, criteria, builder, queryType);
        } 
        return predicate;
    }
     ...
     */
    private <R,T> Predicate createPredicateFromIntersectingCriteriaList(CriteriaQuery<R> query, List<QueryCriteria> intersectingCriteriaList, CriteriaBuilder builder, Class<T> queryType ) {
        ...
        for( QueryCriteria intersectingCriteria : intersectingCriteriaList ) { 
            Predicate predicate = createPredicateFromSingleOrGroupCriteria(query, intersectingCriteria, builder, queryType);
     ...
     */
    private <R,T> Predicate createPredicateFromSingleCriteria(CriteriaQuery<R> query, QueryCriteria criteria, CriteriaBuilder builder, Class<T> queryType ) {
        Predicate predicate = null;
        assert criteria.hasValues() || criteria.hasDateValues() : "No values present for criteria with list id: [" + criteria.getListId() + "]";

        Root<?> table = getTable(query, queryType);
        Expression entityField = getEntityField(query, queryType, criteria.getListId());
        
        if( entityField != null ) { 
            predicate = basicCreatePredicateFromSingleCriteria(builder, entityField, criteria);
        } else { 
           predicate = implSpecificCreatePredicateFromSingleCriteria(query, criteria, builder, table, queryType );
        }
        
        return predicate;
    }

    protected <T> Root getTable(CriteriaQuery<T> query, Class queryType) { 
        Root<?> table = null;
        for( Root<?> root : query.getRoots() ) { 
           if( root.getJavaType().equals(queryType) )  { 
               table = root;
    ...
    protected Predicate basicCreatePredicateFromSingleCriteria(CriteriaBuilder builder, Expression entityField, QueryCriteria criteria) { 
        Predicate predicate = null;
